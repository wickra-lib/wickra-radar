# Fuzzing Wickra Radar

[`cargo-fuzz`](https://rust-fuzz.github.io/book/cargo-fuzz.html) harnesses for the parsing and stateful entry points of Wickra Radar. Fuzzing requires a nightly Rust toolchain; CI runs every target for 30 seconds on the family's pinned `nightly-2026-07-01`.
...
| --- | --- |
| `spec_parse` | The spec-parsing path: arbitrary bytes are parsed as a radar spec (JSON and TOML) and as a config. |
| `command_json` | The JSON command boundary: a radar built from a fixed valid spec is driven with arbitrary bytes as a command. |
| `scan` | The batch scan: a `{spec, events}` object is parsed and scanned. |
...
cargo +nightly-2026-07-01 fuzz run --target x86_64-unknown-linux-gnu scan
```
