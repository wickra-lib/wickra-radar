...
/**
 * A radar instance driven by JSON commands, over the Wickra C ABI (FFM/Panama).
 ...
 */
public final class Radar implements AutoCloseable {
    ...
    public Radar(String specJson) {
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (created.address() == 0) {
                throw new IllegalArgumentException("wickra-radar: invalid spec");
            }
            this.handle = created;
    ...
    public String command(String cmdJson) {
        if (handle == null) {
            throw new IllegalStateException("radar is closed");
        }
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment cmd = arena.allocateFrom(cmdJson);
            int len = (int) Native.COMMAND.invokeExact(handle, cmd, MemorySegment.NULL, 0L);
            if (len < 0) {
                throw new IllegalStateException("wickra-radar: command failed (code " + len + ")");
            }
