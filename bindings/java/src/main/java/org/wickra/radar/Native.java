...
/** Raw FFM (Panama) downcall surface for the wickra-radar C ABI. */
final class Native {
    private Native() {}
    ...
    static final MethodHandle NEW =
            handle("wickra_radar_new", FunctionDescriptor.of(C_PTR, C_PTR));
    static final MethodHandle FREE =
            handle("wickra_radar_free", FunctionDescriptor.ofVoid(C_PTR));
    static final MethodHandle COMMAND =
            handle("wickra_radar_command", FunctionDescriptor.of(C_INT, C_PTR, C_PTR, C_PTR, C_LONG));
    static final MethodHandle VERSION =
            handle("wickra_radar_version", FunctionDescriptor.of(C_PTR));
    ...
    private static SymbolLookup loadLibrary() {
        String libFile = System.mapLibraryName("wickra_radar");
        ...
        throw new UnsatisfiedLinkError("wickra-radar: could not load the native library (" + libFile
    ...
    private static Path extractBundled(String libFile) {
        ...
        try (InputStream in = Native.class.getResourceAsStream(resource)) {
            ...
            Path tmp = Files.createTempFile("wickra-radar-", "-" + libFile);
