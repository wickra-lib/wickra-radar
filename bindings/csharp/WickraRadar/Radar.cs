...
namespace Wickra.Radar;

/// <summary>
/// A radar instance driven by JSON commands, over the Wickra C ABI. Build one
...
{
    private readonly RadarHandle _handle;

    /// <summary>Build a radar from a spec JSON string.</summary>
    /// <exception cref="ArgumentException">The spec is null/invalid.</exception>
    public Radar(string specJson)
    {
        IntPtr ptr = Native.wickra_radar_new(Utf8(specJson));
        if (ptr == IntPtr.Zero)
        {
            throw new ArgumentException("wickra-radar: invalid spec", nameof(specJson));
        }
        _handle = new RadarHandle(ptr);
    }
    ...
    public string Command(string cmdJson)
    {
        ...
        int n = Native.wickra_radar_command(h, cmd, null, 0);
        if (n < 0)
        {
            throw new InvalidOperationException($"wickra-radar: command failed (code {n})");
        }
        var buf = new byte[n + 1];
        Native.wickra_radar_command(h, cmd, buf, (nuint)buf.Length);
        return Encoding.UTF8.GetString(buf, 0, n);
    }

    /// <summary>The library version.</summary>
    public static string Version() =>
        Marshal.PtrToStringUTF8(Native.wickra_radar_version()) ?? string.Empty;

    /// <summary>Free the native radar handle.</summary>
...
/// <summary>A safe handle owning a native radar pointer.</summary>
internal sealed class RadarHandle : SafeHandle
{
    public RadarHandle(IntPtr handle)
    ...
    {
        Native.wickra_radar_free(handle);
        return true;
    }
}
