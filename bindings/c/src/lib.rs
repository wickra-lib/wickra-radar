//! The wickra-radar C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `Radar::command_json`: construct a `Radar` from a spec JSON, drive it with
//! command JSONs and read back response JSONs, and free the handle. No radar type
...
//!
//! 1. Call [`wickra_radar_command`] with `out = NULL`, `cap = 0` to learn the
...
pub const WICKRA_RADAR_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_RADAR_ERR_PANIC: i32 = -3;
...
/// Construct a radar from a spec JSON string (`""` or `"{}"` for an empty
...
/// not a valid spec. Free the handle with [`wickra_radar_free`].
///
/// # Safety
/// `spec_json` must be null or a valid NUL-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn wickra_radar_new(spec_json: *const c_char) -> *mut WickraRadar {
...
/// Destroy a radar handle. Null is a no-op.
///
/// # Safety
/// `handle` must be null or a handle previously returned by [`wickra_radar_new`]
/// and not already freed.
#[no_mangle]
pub unsafe extern "C" fn wickra_radar_free(handle: *mut WickraRadar) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_radar_command(
    handle: *mut WickraRadar,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_RADAR_ERR_NULL;
    }
...
#[no_mangle]
pub extern "C" fn wickra_radar_version() -> *const c_char {
...
mod tests {
    ...
    const SPEC: &str = r#"{"signals":[{"kind":"funding_flip","params":[0.0005]}]}"#;
    ...
    fn new_command_free_round_trip() {
        ...
        let len = unsafe { wickra_radar_command(handle, cmd.as_ptr(), ptr::null_mut(), 0) };
        assert!(len > 0);
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_radar_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_spec_yields_null_handle() {
        let handle = unsafe { wickra_radar_new(ptr::null()) };
        assert!(handle.is_null());
    }
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code =
            unsafe { wickra_radar_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_RADAR_ERR_NULL);
        // Null command with a valid handle.
        let spec = CString::new(SPEC).unwrap();
        let handle = unsafe { wickra_radar_new(spec.as_ptr()) };
        let code = unsafe { wickra_radar_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_RADAR_ERR_NULL);
        unsafe { wickra_radar_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_radar_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_radar_version();
